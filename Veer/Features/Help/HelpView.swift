        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Group {
                    ...
                    bullet("**↓** (card view) or **→** (vertical list) reveals the actions for the selected clip and steps between them — no mouse needed.")
                    bullet("**↑** (card view) or **←** (vertical list) closes the action strip.")
                ...
                Text("Smart actions")
                    .font(.title2)
                    .padding(.top, 8)
                Text("When a clip is exactly a URL, email address, phone number, or hex color, an action strip appears right on the clip — no separate window. Reveal it with ↓ (cards) or → (list), step between actions with the same key, and press Return to run the highlighted one. ↑ / ← closes the strip. Mouse users can right-click a clip and pick from the Smart Actions menu instead. Running any action dismisses the panel; copy actions put the value on the clipboard and return you to your app, ready to paste.")
                Text("Permissions")
                    .font(.title2)
                    .padding(.top, 8)
